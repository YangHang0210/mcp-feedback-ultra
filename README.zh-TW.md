...
注意在任務完成前根據mcp-feedback-ultra工具的要求進行工具調用
```
...
```bash
# 版本查詢
uvx mcp-feedback-ultra@latest version       # 檢查版本

# 介面測試
uvx mcp-feedback-ultra@latest test --web    # 測試 Web UI (自動持續運行)
uvx mcp-feedback-ultra@latest test --desktop # 測試桌面應用程式 (v2.5.0 新功能)

# 調試模式
MCP_DEBUG=true uvx mcp-feedback-ultra@latest test

# 指定語言測試
MCP_LANGUAGE=en uvx mcp-feedback-ultra@latest test --web    # 強制使用英文介面
MCP_LANGUAGE=zh-TW uvx mcp-feedback-ultra@latest test --web  # 強制使用繁體中文
MCP_LANGUAGE=zh-CN uvx mcp-feedback-ultra@latest test --web  # 強制使用簡體中文
```
...
# 或直接使用指令
uv run python -m mcp_feedback_ultra test              # 標準功能測試
uvx --no-cache --with-editable . mcp-feedback-ultra test --web   # Web UI 測試 (持續運行)
uvx --no-cache --with-editable . mcp-feedback-ultra test --desktop # 桌面應用測試
...
A: v2.4.0 版本已完全移除 PyQt6 GUI 依賴。如需使用舊版 GUI，請指定 v2.3.0 或更早版本：`uvx mcp-feedback-ultra@2.3.0`
...
**Q: 中文字符亂碼**
A: 已在 v2.0.3 修復。更新到最新版本：`uvx mcp-feedback-ultra@latest`

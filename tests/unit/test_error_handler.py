...
class TestErrorHandler:
    ...
    def test_classify_error_network(self):
        ...
        # 测试包含網絡關鍵字的错误（不包含 timeout）
        # 修復 assignment 错误 - 使用正確的異常類型
    ...
    def test_get_error_solutions(self):
        ...
        assert any(
            ...
            for keyword in ["網絡", "网络", "network", "连接", "连接", "connection"]
        )
    ...
    def test_language_support(self, mock_get_message):
        ...
        # 测试繁體中文
        mock_get_message.return_value = "網絡连接出現問題"
        result = ErrorHandler.format_user_error(error)
        assert "網絡连接出現問題" in result
    ...
    def test_get_current_language_fallback(self):
        """测试語言獲取回退機制"""
        # 由於 i18n 系統可能會覆蓋环境变量，我們主要测试函數不會拋出異常

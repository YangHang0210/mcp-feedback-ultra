...
MCP Feedback Ultra 支援多種部署環境，具備智能環境檢測和自適應配置能力。
...
```bash
# 直接運行
uvx mcp-feedback-ultra@latest web

# 指定版本
uvx mcp-feedback-ultra@2.4.3 web
```

#### 2. 使用 pip
```bash
# 安裝
pip install mcp-feedback-ultra

# 運行
mcp-feedback-ultra web
```
...
# 運行
uv run python -m mcp_feedback_ultra web
```
...
```bash
# 在遠程服務器運行
mcp-feedback-ultra web --host 0.0.0.0 --port 8000
```
...
RUN pip install mcp-feedback-ultra

EXPOSE 8000

CMD ["mcp-feedback-ultra", "web", "--host", "0.0.0.0", "--port", "8000"]
```

```bash
# 構建和運行
docker build -t mcp-feedback-ultra .
docker run -p 8000:8000 mcp-feedback-ultra
```
...
```bash
mcp-feedback-ultra web [OPTIONS]
```
...
# 解決方案：使用不同埠
mcp-feedback-ultra web --port 8001
```

2. **瀏覽器無法開啟**
```bash
# 手動開啟瀏覽器
mcp-feedback-ultra web --no-browser
...
```bash
# 啟用詳細日誌
mcp-feedback-ultra web --debug

# 查看詳細錯誤信息
export PYTHONPATH=.
python -m mcp_feedback_ultra.debug
```
...
# 或使用 pip 升級
pip install --upgrade mcp-feedback-ultra==2.4.3
```
...
```bash
# 停止服務
pkill -f mcp-feedback-ultra

# 安裝舊版本
pip install mcp-feedback-ultra==2.4.2

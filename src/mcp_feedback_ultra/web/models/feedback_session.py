...
class WebFeedbackSession:
    ...
    def __init__(
        ...
        self.user_messages: list[dict] = []  # 用戶消息记录
        ...
        self.last_heartbeat = None  # 记录最後一次心跳時間
        ...
        self.user_timeout_seconds = 3600  # 预设 1 小時
    ...
    def set_error(self, message: str = "會話發生错误") -> bool:
        """设置错误狀態（特殊方法，可從任何狀態進入）"""
        old_status = self.status
    ...
    def _schedule_auto_cleanup(self):
        ...
        debug_log(
            f"會話 {self.session_id} 自動清理定時器已设置，{self.auto_cleanup_delay}秒後觸發"
        )
    ...
    def update_timeout_settings(self, enabled: bool, timeout_seconds: int = 3600):
        ...
        """
        debug_log(f"更新會話超時设定: enabled={enabled}, seconds={timeout_seconds}")
        ...
        if enabled and self.status == SessionStatus.WAITING:
            ...
            def timeout_handler():
                debug_log(f"用戶设定的超時已到: {self.session_id}")
                # 设置超時標誌
                self.status = SessionStatus.TIMEOUT
                self.status_message = "用戶设定的會話超時"
                # 设置完成事件，讓 wait_for_feedback 結束等待
                self.feedback_completed.set()

            self.user_timeout_timer = threading.Timer(timeout_seconds, timeout_handler)
            self.user_timeout_timer.start()
            debug_log(f"已启动用戶超時計時器: {timeout_seconds}秒")
        ...
        try:
            ...
            if completed:
                ...
                if self.status == SessionStatus.TIMEOUT and self.user_timeout_enabled:
                    debug_log(f"會話 {self.session_id} 因用戶设定超時而結束")
            ...
            # 超時了，立即清理资源
            debug_log(
                f"會話 {self.session_id} 在 {actual_timeout} 秒後超時，開始清理资源..."
            )
            await self._cleanup_resources_on_timeout()
            raise TimeoutError(
        ...
        if self.websocket:
            try:
                ...
                if os.environ.get("MCP_DESKTOP_MODE", "").lower() == "true":
                    debug_log("桌面模式：反饋提交後立即关闭桌面应用程序")
                    ...
                    try:
                        ...
                        debug_log("桌面应用程序立即关闭成功")
                    except Exception as close_error:
                        debug_log(f"立即关闭桌面应用程序失敗: {close_error}")
    ...
    async def _cleanup_resources_enhanced(self, reason: CleanupReason):
        ...
        debug_log(f"開始清理會話 {self.session_id} 的资源，原因: {reason.value}")
        ...
        try:
            ...
            if self.websocket:
                try:
                    ...
                    await asyncio.sleep(0.1)  # 給前端一點時間处理消息

                    # 安全关闭 WebSocket
                    await self._safe_close_websocket()
                    debug_log(f"會話 {self.session_id} WebSocket 已关闭")
                    resources_cleaned += 1
                except Exception as e:
                    debug_log(f"关闭 WebSocket 時發生错误: {e}")
                finally:
                    self.websocket = None
            ...
            debug_log(
                f"會話 {self.session_id} 资源清理完成，耗時: {cleanup_duration:.2f}秒，"
                f"清理资源: {resources_cleaned}個，釋放內存: {memory_freed}字節"
            )
            ...
            debug_log(
                f"清理會話 {self.session_id} 资源時發生错误 [错误ID: {error_id}]: {e}"
            )
    ...
    def _cleanup_sync(self):
        """同步清理會話资源（但保留 WebSocket 连接）- 保持向後兼容"""
    ...
    def _cleanup_sync_enhanced(
        ...
        cleanup_start_time = time.time()
        debug_log(
            f"同步清理會話 {self.session_id} 资源，原因: {reason.value}，保留WebSocket: {preserve_websocket}"
        )
        ...
        try:
            ...
            debug_log(
                f"同步清理會話 {self.session_id} 资源時發生错误 [错误ID: {error_id}]: {e}"
            )
    ...
    async def _safe_close_websocket(self):
        ...
        try:
            ...
            debug_log(f"會話 {self.session_id} WebSocket 已正常关闭")

        except TimeoutError:
            debug_log(f"會話 {self.session_id} WebSocket 关闭超時")
        except RuntimeError as e:
            if "attached to a different loop" in str(e):
                debug_log(
                    f"會話 {self.session_id} WebSocket 事件循環衝突，忽略关闭错误: {e}"
                )
            else:
                debug_log(f"會話 {self.session_id} WebSocket 关闭時發生运行時错误: {e}")
        except Exception as e:
            debug_log(f"會話 {self.session_id} 关闭 WebSocket 時發生未知错误: {e}")
